...
import { getRdvs } from "../get/rdv";
import { format } from "date-fns";

const createRdv = async (prevState: PrevState, formData: FormData) => {
  try {
...
const notifyRdv = async () => {
  try {
    ...
    if (!rdvs.rdvs || (rdvs.rdvs && rdvs.rdvs.length === 0)) {
